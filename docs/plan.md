...
**Now:** Phase 25 — Bulk Data Import: in-app master-sheet CSV importer **shipped at field 0.25.0** (parse → preview → skip-if-exists apply; derives sessions/bands/records, stub locations, soft warnings + rejects CSV). Verified against the real 342-row master sheet (37 sessions / 342 bands / 330 records / 0 rejects). **Next up (research done, not started): lost/destroyed-band records** — band-event rows currently vanish from the session; emit them as `BirdRecord`s marked with capture code `L`/`D` (Option 2). Full findings + implementation checklist in [research-destroyed-bands.md](apps/field/research-destroyed-bands.md). Also remaining: the Hallie confirmations (agency-export `S covs`/`G covs`, Alula, any extra IBP letters) — see Phase 25 below. _Update this line whenever the active phase changes._
