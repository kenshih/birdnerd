...
| 32.0–32.4 | Partially complete | Field 0.32.0 made `npm run dev` local-first with an explicit hosted-pilot command; 0.32.1 added a guarded disposable operational fixture; 0.32.2 added real local fixture-member Auth; 0.32.3 added visible identity and provider-neutral sign-out; and 0.32.4 added separate-client local Google OAuth plus exact-email fixture invitation. Local Auth, Provisioner, Event admission, and replay remain real; hosted Supabase stays unchanged. `sync-db` removal and the UX-regression gate remain in active Phase 32. |
