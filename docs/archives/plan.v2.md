...
| 32.0–32.1 | Partially complete | Field 0.32.0 made `npm run dev` local-first with an explicit hosted-pilot command. Field 0.32.1 added a guarded, disposable local operational fixture that exercises real local Auth, Provisioner, Event admission, and replay. The remaining local-auth, identity/sign-out, local-Google, `sync-db` removal, and UX-regression work stays in the active Phase 32 roadmap. |
