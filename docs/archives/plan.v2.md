...
| 32.0–32.5 | Partially complete | Field 0.32.0 made `npm run dev` local-first with an explicit hosted-pilot command; 0.32.1 added a guarded disposable operational fixture; 0.32.2 added real local fixture-member Auth; 0.32.3 added visible identity and provider-neutral sign-out; 0.32.4 added separate-client local Google OAuth plus exact-email fixture invitation; and 0.32.5 removed the obsolete `apps/sync-db` experiment, its workspace hooks, and stale PWA/deployment documentation. Local Auth, Provisioner, Event admission, and replay remain real; hosted Supabase stays unchanged. The UX-regression gate remains in active Phase 32. |
