...
| 32 | Complete | Field 0.32.0–0.32.6 established a local-first Field development environment: guarded local Supabase and a disposable operational fixture, real fixture-member Auth, visible identity/sign-out, and separately configured local Google OAuth; hosted Supabase remains unchanged. It removed the obsolete `apps/sync-db` experiment and added a repeatable UX-regression gate: concise workflow contracts with production Storybook examples, direct rules, and browser evidence. Field 0.32.6 restored non-blocking event-backed Record validation feedback. |
