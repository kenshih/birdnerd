...
export default function OperationalWorkspacePage({ onHome }: { onHome: () => void }) {
  ...
  function createStation(event: FormEvent) { event.preventDefault(); void perform(async () => { await decide({ kind: 'create', entity_kind: 'station', fields: compact({ name: stationName.trim() }) }); setStationName('') }) }
  function createNet(event: FormEvent) { event.preventDefault(); if (!netStationId) return; void perform(async () => { await decide({ kind: 'create', entity_kind: 'net', station_id: netStationId, fields: compact({ label: netLabel.trim() }) }); setNetLabel('') }) }
  function receiveBand(event: FormEvent) { event.preventDefault(); void perform(async () => { await decide({ kind: 'receive-bands', bands: [{ band_number: bandNumber.trim() }] }); setBandNumber('') }) }
  function createPerson(event: FormEvent) { event.preventDefault(); void perform(async () => { await decide({ kind: 'create', entity_kind: 'person', fields: compact({ name: personName.trim(), initials: personInitials.trim().toUpperCase() }) }); setPersonName(''); setPersonInitials('') }) }
  function createBander(event: FormEvent) { event.preventDefault(); if (!banderPersonId) return; void perform(async () => { await decide({ kind: 'create', entity_kind: 'bander', person_id: banderPersonId, fields: { role: banderRole } }); setBanderPersonId('') }) }
