...
import { legacyBandLabel, recordBandReference, unresolvedManagedBandLabel, type RecordBandMode } from '../utils/recordReference'
...
export default function OperationalWorkspacePage({ onHome, initialRecordId, onCloseRecordView }: Props) {
  ...
  const [stationAgencyCode, setStationAgencyCode] = useState('')
  ...
  const recordViewManagedBand = viewingRecordId && managedBandId ? projection.entities.get(managedBandId) : undefined
  const recordViewBands = recordViewManagedBand?.kind === 'band' && !bands.some(band => band.id === recordViewManagedBand.id) ? [...bands, recordViewManagedBand] : bands
  const recordViewNetId = viewingRecordId ? text(recordDraft.net_id) : ''
  const recordViewNet = recordViewNetId ? projection.entities.get(recordViewNetId) : undefined
  const recordViewNets = recordViewNet?.kind === 'net' && !stationNets.some(net => net.id === recordViewNet.id) ? [...stationNets, recordViewNet] : stationNets
  const recordViewBanderId = viewingRecordId ? text(recordDraft.bander_id) : ''
  const recordViewBander = recordViewBanderId ? projection.entities.get(recordViewBanderId) : undefined
  const recordViewBanders = recordViewBander?.kind === 'bander' && !banders.some(bander => bander.id === recordViewBander.id) ? [...banders, recordViewBander] : banders
  ...
  function createRecord(event: FormEvent) {
    ...
    if (!sessionId) return
    void perform(async () => {
      const bandSelection = currentBandSelection(bandMode, foreignBandNumber, managedBandId, bands)
      const fields = { ...eventFields(recordDraft, RECORD_KEYS), ...(bandSelection ? { band_selection: bandSelection } : {}) }
      if (editingRecordId) {
  ...
  function beginRecordEdit(id: string) {
    ...
    setEditingRecordId(id); setRecordDraft(draftFrom(record.fields, RECORD_KEYS)); setBandMode(reference.mode); setForeignBandNumber(reference.bandNumber); setManagedBandId(reference.bandId)
  }
  const beginRecordView = useCallback((id: string) => {
    const record = projection.entities.get(id)
    if (!record) return
    const reference = recordBandReference(record.fields)
    setTab('records'); setSessionId(text(record.fields.session_id)); setEditingRecordId(''); setViewingRecordId(id); setRecordDraft(draftFrom(record.fields, RECORD_KEYS)); setBandMode(reference.mode); setForeignBandNumber(reference.bandNumber); setManagedBandId(reference.bandId)
  }, [projection])
  ...
  function createStation(event: FormEvent) { event.preventDefault(); void perform(async () => { const fields = { name: stationName.trim() || null, agency_code: stationAgencyCode.trim().toUpperCase() || null }; if (editingStationId) await decide({ kind: 'amend', entity_kind: 'station', entity_id: editingStationId, fields }); else await decide({ kind: 'create', entity_kind: 'station', fields: compact(fields) }); setStationName(''); setStationAgencyCode(''); setEditingStationId('') }) }
  ...
  function beginConfigurationEdit(entity: OperationalEntity) {
    if (entity.kind === 'station') { setEditingStationId(entity.id); setStationName(text(entity.fields.name)); setStationAgencyCode(text(entity.fields.agency_code)); return }
  ...
  return <main style={styles.page}>
    ...
    {tab === 'configuration' && (access.role === 'admin' ? <>
      <form style={styles.card} onSubmit={createStation}><h2>{editingStationId ? 'Amend Station' : 'Station'}</h2><label>Name<input value={stationName} onChange={event => setStationName(event.target.value)} /></label><label>Agency code<input value={stationAgencyCode} onChange={event => setStationAgencyCode(event.target.value.toUpperCase())} maxLength={4} pattern="[A-Z]{4}" title="Use four uppercase letters, such as GCFS." placeholder="GCFS" /></label><p style={styles.hint}>Optional four-letter local code used in agency CSV exports.</p><button disabled={saving}>Save offline</button></form>
      ...
      <EntityList title="Stations" entities={entities.filter(entity => entity.kind === 'station')} detail={entity => [text(entity.fields.agency_code), text(entity.fields.name) || entity.id].filter(Boolean).join(' — ')} onCorrect={id => { const entity = projection.entities.get(id); if (entity) beginConfigurationEdit(entity) }} onDeactivate={entity => void perform(() => decide({ kind: entity.active ? 'deactivate' : 'reactivate', entity_kind: 'station', entity_id: entity.id }))} />
...
function recordViewBanderLabel(bander: OperationalEntity, entities: ReadonlyMap<string, OperationalEntity>): string { const person = entities.get(text(bander.fields.person_id)); const personLabel = `${text(person?.fields.initials) || '—'} — ${text(person?.fields.name) || 'Unresolved Person'}${person && !person.active ? ' (inactive Person)' : ''}`; return `${personLabel} (${text(bander.fields.role) || 'Bander'})${bander.active ? '' : ' (inactive Bander)'}` }
function recordLabel(record: OperationalEntity, bands: readonly OperationalEntity[]): string { const reference = recordBandReference(record.fields); const band = reference.mode === 'managed' ? text(bands.find(item => item.id === reference.bandId)?.fields.band_number) || unresolvedManagedBandLabel(reference.bandNumber, reference.bandId) : reference.mode === 'foreign' ? reference.bandNumber || 'Foreign Band' : reference.mode === 'legacy' ? legacyBandLabel(reference.bandNumber) : reference.mode === 'unbanded' ? 'Unbanded' : ''; return [text(record.fields.species_code) || 'Species not entered', band, text(record.fields.capture_code) === 'R' ? 'recap' : ''].filter(Boolean).join(' · ') }
