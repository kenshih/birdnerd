...
export default function OperationalWorkspacePage({ onHome }: { onHome: () => void }) {
  ...
  return <main style={styles.page}>
    ...
    <section style={styles.status}><strong>{access.workspace_name}</strong><span>{statusText(status)}</span><button type="button" onClick={() => void synchronize(true)}>Sync now</button></section>
...
function statusText(status: SyncStatus) { return status.kind === 'syncing' ? 'Syncing…' : status.kind === 'offline' ? `Offline — changes stay on this device (${status.message})` : status.kind === 'deferred' ? `Waiting to retry ${status.deferred} Event${status.deferred === 1 ? '' : 's'} (${status.message})` : status.kind === 'attention' ? `${status.rejected} Events need attention` : status.last_synced_at ? `Synced ${new Date(status.last_synced_at).toLocaleTimeString()}` : 'Ready to sync' }
